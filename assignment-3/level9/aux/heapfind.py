...
if stack_start is not None and stack_end is not None:
    print(f"Heap region: {hex(stack_start)} - {hex(stack_end)}")
...
for i in range(start, end + 1):
    try:
        res = gdb.execute(f"x/1gx state->messages[{i}]", to_string=True)      
        message_ptr = res  
        ...
        if stack_start <= message_address <= stack_end:
            print(f"[{i}] message_ptr {hex(message_address)} is in the heap region, difference from messages[0]: {hex(abs(message_address - messages_0_ptr))}")
            print("res > ", res)
